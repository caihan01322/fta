...
def convert_tree2graph_reverse(tree_json):
    idx_tree2graph_map = {1: 0}
    graph_json = [({}, tree_json["1"])]
    for tree_idx in sorted(map(int, tree_json.keys()))[1:]:
        status = "failure" if tree_idx % 2 else "success"
        solution_id = tree_json[str(tree_idx)]
        graph_idx = idx_tree2graph_map[tree_idx / 2]
        idx_tree2graph_map[tree_idx] = len(graph_json)
        graph_json.append(({graph_idx: [status]}, solution_id))
    return graph_json
