...
class ComponentAPI(object):
    ...
    def __call__(self, *args, **kwargs):
        try:
            ...
            logger.exception("%s %s", constants.ERROR_02_ESB, '\n'.join(log_message))
    ...
    def _call(self, *args, **kwargs):
        ...
        try:
            ...
            logger.exception('%s Error occurred when requesting method=%s url=%s',
                             constants.ERROR_02_ESB, self.method, self.url)
        ...
        try:
            ...
            if not json_resp['result']:
                ...
                log_message = (u"%(error_code)s 组件返回错误信息: %(message)s, request_id=%(request_id)s "
                               u"url=%(url)s params=%(params)s data=%(data)s") % {
                    'error_code': constants.ERROR_02_THIRD,
