...
class FtaGCloudClient(object):
    ...
    def __init__(
        ...
        self.app_secret = app_secret or getattr(settings, "SECRET_KEY", None)
        self.headers = headers or {}
        self.common_args = common_args or {}
        self.timeout = timeout
        self.endpoint = endpoint or getattr(
            settings, "GCLOUD_ENDPOINT", "",
        )
        self.detail_link = detail_link or urlparse.urljoin(getattr(
            settings, "GCLOUD_DETAIL_ENDPOINT", ""), "taskflow/detail/{task_id}/{cc_biz_id}/")

        self.create_task = partial(
            self.request, method="POST",
            path="task_api/create_task/{biz_cc_id}/",
        )
        self.get_task_status = partial(
            self.request, method="GET",
            path="task_api/get_task_status/{biz_cc_id}/",
        )
        self.run_task = partial(
            self.request, method="POST",
            path="task_api/run_task/{biz_cc_id}/",
        )
        self.get_tmpl = partial(
            self.request, method="GET",
            path="tmpl_api/get_tmpl/{biz_cc_id}/",
        )
        self.get_tmpl_info = partial(
            self.request, method="GET",
            path="tmpl_api/get_tmpl_info/{biz_cc_id}/",
        )
    ...
    def request(self, kwargs=None, path="", method="GET"):
        url = urlparse.urljoin(self.endpoint, path).format(
            biz_cc_id=self.cc_biz_id,
        )
