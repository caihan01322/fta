...
logger = logging.getLogger('poll_alarm.event.icinga2')


class Icinga2EventAlarm(BasePollAlarm):
    """主动推送消息，请勿添加到POLL_LIST
    """
    SOURCE_TYPE = 'ICINGA2'

    def __init__(self, alarm, cc_biz_id):
        '''raw_data is a json map object:
        {
        ...
        '''
        super(Icinga2EventAlarm, self).__init__()
        self.cc_biz_id = cc_biz_id
        self.alarm = alarm
        logger.debug('icinga2 get data is: %s' % self.alarm)

    def pull_alarm(self):
        if self.alarm.get("is_base64"):
            self.alarm["alarm_content"] = base64.b64decode(
                self.alarm["alarm_content"],
            )
