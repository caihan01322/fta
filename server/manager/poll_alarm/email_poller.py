...
from manager.utils.query_cc import (handle_alarm_source_empty,
...
class EMail(dict):
    AVALIABLE_CONTENT_TYPE = frozenset([
...
class MailPoller(object):
    ...
    def search(
        ...
        if before:
            queries.append(
                '(BEFORE "%s")' % before.replace(days=1).strftime("%d-%b-%Y"),
            )
        if since:
            queries.append(
    ...
    def filter(
            self, mails, sent_from=None, sent_to=None, subject=None,
            before=None, since=None, fetch_header_already=False,
    ):
        result_list = []
        mails = mails if fetch_header_already else self.iter_fetch_chunks(
            mails=mails, criteria=self.CRITERIA_HEADER,
        )
    ...
    def fetch_header(self, mails):
        for mail in self.iter_fetch_chunks(
                mails=mails, criteria=self.CRITERIA_HEADER,
        ):
            pass
    ...
    def fetch(self, mails, criteria=None):
        ...
        status, result = self.imap_client.fetch(
            ",".join(mail_mappings.keys()), criteria or self.CRITERIA,
        )
        if status != "OK":
    ...
    def iter_fetch_chunks(self, mails, chunks=100, criteria=None):
        ...
        if mails:
            length = len(mails)
            chunks_list = [
                mails[i: i + chunks] for i in range(0, length, chunks)
            ]
    ...
    def fetch_by(
        ...
        mails = self.filter(
            mails, sent_from=sent_from, sent_to=sent_to, subject=subject,
...
class EMailPollAlarm(BasePollAlarm):
    SOURCE_TYPE = "EMAIL"
    ALARM_FIELDS = [
        "ip", "cc_biz_id", "alarm_content",
        "alarm_type", "source_id", "source_time",
        "application_id",
    ]
    SIZE_LIMIT = 200 * 1024

    def __init__(
            self, force_begin_time=None, force_end_time=None,
            minutes=None, delta_minutes=0, debug=False,
    ):
        """
    ...
    def load_application_config(self):
        ...
        for app in session.query(AlarmApplication).filter(
                AlarmApplication.cc_biz_id != 0,
                AlarmApplication.is_enabled is True,
            ...
            try:
                ...
                logger.warning("load application[%s] config failed, error message %s", app.id, err)
            if app.extra_config:
            ...
            for app in applications:
                ...
                try:
                    filter_results = poller.filter(
                        ...
                        sent_from=(
                            sent_from.split(",")
                            if sent_from else None
                        ),
                        sent_to=(
                            sent_to.split(",")
                            if sent_to else None
                        ),
                    )
                except Exception as error:
    ...
    def get_host_info(self, cc_biz_id, ip):
        ...
        if not host_info:
            host_info = query_cc.get_host_info_by_ip(
                cc_biz_id, ip,
            )
            self.hosts_info[cache_key] = host_info
        return host_info

    def get_company_info(self, cc_biz_id, ip):
        cache_key = (cc_biz_id, ip)
        company_info = self.companies_info.get(cache_key)
        if not company_info:
            company_info = query_cc.get_company_info_by_ip(
                cc_biz_id, ip,
            )
    ...
    def match_alarm_from_emails(self, emails, application):
        ...
        if item_config:
            for mail in emails:
                ...
                try:
                    context = parser.parse(email.content)
                except Exception as error:
                ...
                values.update({
                    ...
                    "source_time": email.time.isoformat(),
                })
                values.setdefault("source_id", email.message_id)

                alarm_list.append({
                    k: values[k]
                    for k in self.ALARM_FIELDS
                })
        return alarm_list
    ...
    def pull_alarm(self):
        ...
        if not self.debug:
            lock_email_polling(
                ...
                self.delta_minutes, group=self.SOURCE_TYPE,
            )
    ...
    def pull_alarm_by_esb(self):
        ...
        for app in self.applications.values():
            ...
            if not app.email_config:
                handle_alarm_source_exception(
                    alram_app_id,
                    _("Mailbox configuration information incomplete")
                )
                continue
        ...
        for i in alarm_list:
            try:
                ...
                company_info = self.get_company_info(cc_biz_id, ip)
                if not company_info:
                    continue
    ...
    def pull_alarm_directly(self):
        ...
        def pull_and_match(item):
            ...
            try:
                mails = pull_emails_by_applications(
                    email_config, applications,
                )
            except Exception as error:
                logger.error(error)
                return ()
            ...
            for app in applications:
                try:
                    results.extend(match_alarm_from_emails(
                        mails, app,
                    ))
                except Exception as error:
                    logger.warning("match email error: %s", error)
        ...
        with self.global_proxy():
            for i in self.pool.imap(
                    pull_and_match, email_config_applitions.items(),
            ):
                if i:
                    alarm_list.extend(i)
    ...
    def clean_alarm_type(self, alarm):
        ...
        return list(monitors.lookup_alarm_type_list(
            [alarm_type],
            cc_biz_id=cc_biz_id, source_type=self.SOURCE_TYPE,
            default=alarm_type,
        ))
...
def lock_email_polling(str_begin_time, str_end_time, delta_minutes, group):
    ...
    if not lock.redis_lock("--lock_%s_%s-%s-%s" % (
            group, str_begin_time, str_end_time, delta_minutes)):
