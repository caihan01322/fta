...
from manager.poll_alarm import icinga2
from manager.www.apiservice import api_page as app

logger = logging.getLogger(__name__)


@app.route("/event/icinga2/<fta_application_id>/", methods=["POST"])
@response.log
@signature_required
def event_icinga2(fta_application_id, cc_biz_id):
    """icinga2上报API
    """
    try:
        # 参数校验
        ...
        event = icinga2.Icinga2EventAlarm(data, cc_biz_id)
        event.start()

        # code 200 OK
        result = {'result': True, 'message': 'push icinga2 event success', 'data': {}, 'code': 1200}
    except validate.ValidateError as error:
        # code 400 Bad Request
        result = {'result': False, 'message': u'%s' % error, 'data': {}, 'code': 1400}
    except Exception as error:
        logger.exception('handle icinga2 event error: %s', error)
        # code 500 Internal Server Error
        result = {'result': False, 'message': 'push icinga2 event error', 'data': {}, 'code': 1500}
