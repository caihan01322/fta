...
from manager.poll_alarm import nagios
from manager.www.apiservice import api_page as app

logger = logging.getLogger(__name__)


@app.route("/event/nagios/<fta_application_id>/", methods=["POST"])
@response.log
@signature_required
def event_nagios(fta_application_id, cc_biz_id):
    """nagios上报API
    """
    try:
        # 参数校验
        ...
        event = nagios.NagiosEventAlarm(data, cc_biz_id)
        event.start()

        # code 200 OK
        result = {'result': True, 'message': 'push nagios event success', 'data': {}, 'code': 1200}
    except validate.ValidateError as error:
        # code 400 Bad Request
        result = {'result': False, 'message': u'%s' % error, 'data': {}, 'code': 1400}
    except Exception as error:
        logger.exception('handle nagios event error')
        # code 500 Internal Server Error
        result = {'result': False, 'message': 'push nagios event error', 'data': {}, 'code': 1500}
