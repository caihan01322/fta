...
var G_V = {
	...
	modal_head : gettext("提示信息"),    //提示框头部信息
...
function alert_message(type){
  if(!type){
    ...
    G_V.modal_head=gettext("提示信息");
  }else if(type==1){
    G_V.modal_ico = "";
    G_V.alert_type = "alert-help";
    G_V.modal_head=gettext("帮助信息");
  }else if(type==2){
    G_V.modal_ico = "";
    G_V.alert_type = "alert-success";
    G_V.modal_head=gettext("完成信息");
  }else if(type==3){
    G_V.modal_ico = "";
    G_V.alert_type = "alert-warning";
    G_V.modal_head=gettext("警告");
  }else if(type==4){
    G_V.modal_ico = "";
    G_V.alert_type = "alert-danger";
    G_V.modal_head=gettext("错误信息");
  }
}
 ...
 * @param  msg:       消息内容
 * @param  type:      消息类型
 * @param  call_back:   点击“确定”后的回调,可以是函数对象，也可以是函数名,但是不能传参数
 */
...
function alert_msg(msg,type,call_back){
	  ...
	  var $this = $(this)
	      ...
	      "<button type='button' class='btn btn-box btn-primary' data-dismiss='modal' onclick='invoke_c_b("+ call_back +");'>" + gettext("确定") + "</button></div></div></div></div>"  //strip for ie7
...
function bk_confirm(msg,call_b_no,call_b_yes){
	  ...
	  //绑定取消回调函数
	  $("<button type='button' class='btn btn-box' data-dismiss='modal' >" + gettext("取消") + "</button>").appendTo($div_footer)
	                                .click(function(){call_b_no.call(this);});
	    
	  //绑定确定回调函数
	  $("<button type='button' class='btn btn-box btn-primary' data-dismiss='modal'>" + gettext("确定") + "</button>").appendTo($div_footer)
 ...
 */
 function add_tab(title_id,tab_title,tab_obj,tab_con_obj){
	 ...
	 if($(tab_obj+" > li").find('a[href="#'+title_id+'"]').attr('href')){
		...
		//添加此tab关联的div内容
		$('<div class="tab-pane fade in active" id="'+title_id+ '"><p>' + gettext('显示动态添加页签的数据内容。') + '</p></div>').appendTo($(tab_con_obj));
	 }
