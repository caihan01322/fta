...
$("#data-" + comtom_name_id).parent('.close-text').addClass('active');

getAvctiveVisible();

// 判断当前选中自愈是否在可见范围
function getAvctiveVisible(){
    if($(".solution-list-table tbody tr.active").length>0){
    var activeHeight = $(".solution-list-table tbody tr.active").offset().top,
...
function del_def(id) {
    var delDialog = new bkDialog({
        ...
        confirmFn: function(){
            $.ajax({
                url: site_url + tip_id + '/del_def/solution/',
                data: {'id': id},
                type: 'POST',
                dataType: 'json',
                success: function(result){
                    ...
                    include_open(tip_id + '/solution_list/');
                }
            });
        },
        okVal: gettext("确定"),
