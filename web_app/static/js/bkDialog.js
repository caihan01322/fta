...
(function(global, factory) {
    ...
    function bkDialog(options) {
        ...
        this.title = opts.title || gettext('确定执行该操作?');        // 弹窗的title
        this.content = opts.content === false ? false : (opts.content || gettext('欢迎使用 bkDialog 组件！'));    // 弹窗的内容
        ...
        this.confirm = opts.confirm || gettext('确定');              // 确定按钮的文字
        this.cancel = opts.cancel || gettext('取消');                // 取消按钮的文字
    ...
    var _initStyle = function() {
        ...
        switch(type) {
            case 'loading':
                ...
                defaultDOM.subTitle = gettext('请稍等...');
                break;
            case 'error':
                defaultDOM.title = gettext('添加用户失败');
                defaultDOM.subTitle = gettext('此窗口<span class="bk-dialog-error-text">3s</span>后关闭');
                icon = this.icon || 'close';
                break;
            case 'success':
                defaultDOM.title = gettext('添加用户成功');
                defaultDOM.subTitle = gettext('<a href="javascript:;" class="bk-dialog-primary-text">继续添加 >></a>');
                icon = this.icon || 'check-1';
                break;
            case 'warning':
                defaultDOM.title = gettext('此操作存在风险');
