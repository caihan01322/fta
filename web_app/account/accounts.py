...
class Account(AccountSingleton):
    ...
    # 平台获取用户信息接口
    BK_GET_USER_INFO_URL = "%s/login/accounts/get_user/" % getattr(
        settings, 'BK_PAAS_INNER_HOST', settings.BK_PAAS_HOST)
    ...
    def is_bk_token_valid(self, request):
        ...
        try:
            ...
            try:
                ...
                logger.error(u"Failed to obtain user info: %s" % e)
        return True, user
    ...
    def verify_bk_login(self, bk_token):
        ...
        # 验证失败
        if not ret:
            logger.info(u"User login token was verified to be invalid: %s" % resp.get('message', ''))
            return False, {}
    ...
    def get_bk_user_info(self, bk_token):
        ...
        # 获取用户信息失败
        if not ret:
            logger.error(u"Failed to request platform interface to obtain user info: %s" % resp.get('message', ''))
            return False, {}
        return True, resp.get('data', {})
