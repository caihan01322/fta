...
def html_escape(html, is_json=False, is_date=False):
    """
...
def render_ajax(result=False, msg="", data=[]):
    """
    返回一个标准的json格式
    """
    feedback = {
    ...
    try:
        feedback_str = json.dumps(feedback, cls=DjangoJSONEncoder)
        return HttpResponse(feedback_str)
    except Exception, e:
        feedback = {
            "result": False,
            "msg": 'format json error : %s \n msg:%s' % (e, msg),
            "data": [],
        }
